...
echo ""
echo "⏰ 3/3 Configurando Cloud Scheduler para ejecutar el radar cada 2 minutos..."
SERVICE_URL=$(gcloud functions describe check-visa-status --region=us-central1 --format="value(serviceConfig.uri)")
echo "🔗 URL del servicio desplegado: $SERVICE_URL"

# Intentar actualizar o crear el trabajo programado
if gcloud scheduler jobs describe ozvisa-radar-job --location=us-central1 &>/dev/null; then
  ...
  echo "🆕 Creando nuevo trabajo programado cada 2 minutos..."
